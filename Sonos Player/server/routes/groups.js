...
module.exports = function Groups(sonosNetwork) {
  ...
  this.router.put("/:id/play/:trackNumber", async (req, res) => {
    await this.sonosNetwork.playTrackFromQueue(
      req.params.id,
      req.params.trackNumber
    );
    res.status(200).send();
  });
  ...
  this.router.put("/:id/queue/remove", async (req, res) => {
    await this.sonosNetwork.removeTracksFromQueue(
      req.params.id,
      req.body.trackIndexes
    );
    res.status(200).send();
  });
  this.router.put("/:id/queue/reorder", async (req, res) => {
    // eslint-disable-next-line max-len
    await this.sonosNetwork.reorderTracksInQueue(
      req.params.id,
      req.body.oldIndex,
      req.body.newIndex
    );
