const { DeviceDiscovery, Listener, Sonos } = require("sonos")
...
class SonosNetwork {
  constructor(socketio, timeout = 5000) {
    ...
    //this._init();
    //this.createSonosNetwork()
    this.socketio.on("connection", () => {
      console.log("<<<<<<<<<SOCKET NETWORK>>>>>>>>>")
      console.log(networkInterfaces)
      console.log("<<<<<<<<<SOCKET NETWORK>>>>>>>>>")
      if (this.initializing) {
        this.socketio.emit(DiscoveringSonosDevices)
      } else if (this.connecting) {
        this.socketio.emit(ConnectingSonosDevices)
  ...
  /*
   * Create Sonos Device using an IP Address
   */
  createSonosNetwork() {
    const device = new Sonos(process.env.SONOS_PORT)
    if (!device) return
    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    ...
    device
      ...
      .then(description => {
        ...
        device.id = UUID
        console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        console.log(device, description, "DEVICE FOUND")
        console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        this.listener
          .subscribeTo(device)
          .then(() => {
            ...
            if (!this.musicLibrary) {
              this.musicLibrary = new MusicLibrary(device)
            }
            ...
            this._parseZoneGroups()
              ...
              .catch(error => {
                this.initializing = false
                console.log("#############################")
                console.log("ZONE GROUPS NOT PARSED", error)
                console.log("#############################")
          ...
          .catch(error => {
            switch (error.statusCode) {
              ...
              default:
                console.log("#############################")
                console.log("LISTENER NOT SUBSCRIBING", error)
                console.log("#############################")
            }
          })
      })
      .catch(error => {
        console.log("#############################")
        console.log("DEVICE NOT CONNECTING", error)
        console.log("#############################")
        this._init()
      })
  }
  ...
  _listen() {
    ...
    this.devices.forEach(device => {
      ...
      device.on("AVTransport", () => {
        this._onAVTransportEvent(device)
      })
      device.on("RenderingControl", () => {
        this._onRenderingControlEvent(device)
      })
      device.on("QueueChanged", () => {
        this._onQueueChangedEvent(device)
      })
    })
    // Global events
    this.listener.on("ZonesChanged", zoneData => {
      this._onZonesChangedEvent(zoneData)
    })
  }

  _onAVTransportEvent(device) {
    // Find the zone group, we only care if it is the coordinator
    const zoneGroup = this.zoneGroups.find(
      zg => zg.coordinator.id === device.id
    )
    if (zoneGroup) {
      this._getAVTransportInfo(device.id).then(transportInfo => {
        this.socketio.emit(SonosEventDataReceived, {
          groupId: zoneGroup.id,
  ...
  _onRenderingControlEvent(device) {
    this._getRenderingControlInfo(device.id).then(renderingInfo => {
      ...
      if (zoneGroup) {
        ...
        const updatedZone = this._updateZoneGroupRenderingInfo(
          zoneGroup.id,
          device.id,
          renderingInfo
        )
  ...
  async getTrackPosition(groupId) {
    const group = this.zoneGroups.find(zg => zg.id === groupId)
    if (group) {
      const track = await group.coordinator.device
        .avTransportService()
        .CurrentTrack()
  ...
  async joinGroup(groupId, zoneId) {
    const zone = this.devices.find(device => device.id === zoneId)
    const group = this.zoneGroups.find(zg => zg.id === groupId)
    await zone.setAVTransportURI({
      uri: `x-rincon:${group.coordinator.id}`,
      onlySetUri: true,
    })
  }
  ...
  _updateZoneGroupTransportInfo(deviceId, data) {
    const index = this.zoneGroups.findIndex(
      group => group.coordinator.id === deviceId
    )
    if (index !== -1) {
      // Merge new data in
  ...
  _updateZoneGroupRenderingInfo(groupId, memberId, data) {
    ...
    if (index !== -1) {
      ...
      if (this.zoneGroups[index].coordinator.id === memberId) {
        this.zoneGroups[index].coordinator = {
          ...this.zoneGroups[index].coordinator,
          ...data,
        }
      } else {
        // otherwise merge it into one of the members
        const mIndex = this.zoneGroups[index].members.findIndex(
      ...
      const renderingInfo = SonosNetwork._getGroupRenderingControlInfo(
        this.zoneGroups[index]
      )
  ...
  async _parseZoneGroups() {
    return new Promise((resolve, reject) => {
      ...
      if (this.devices.length === 0) {
        reject(new Error(NoDevicesFound))
      }
      let zoneGroups = []

      this.devices[0]
        .getAllGroups()
        .then(async rawGroups => {
          ...
          zoneGroups = zoneGroups.filter(
            group => group.coordinator.device !== undefined
          )

          // Fetch the transport info for all zones
          await Promise.all(
            zoneGroups.map(async (group, index) => {
              const transportInfo = await this._getAVTransportInfo(
              ...
              await Promise.all(
                group.members.map(async (member, mIndex) => {
              ...
              const renderingInfo =
                SonosNetwork._getGroupRenderingControlInfo(group)
          ...
          zoneGroups.forEach(group =>
            group.members.sort((m1, m2) => m1.name > m2.name)
          )
          // Sort all the zones alphabetically by coordinator
          zoneGroups.sort(
            (group1, group2) =>
              group1.coordinator.name > group2.coordinator.name
          )
  ...
  async _getURIFromData(coordinatorId, data) {
    ...
    if (data.artistPath) {
      uri = encodeURI(
        `x-rincon-playlist:${coordinatorId}#A:ALBUMARTIST/${data.artistPath}`
      )
    } else if (data.genrePath) {
      uri = encodeURI(
        `x-rincon-playlist:${coordinatorId}#A:GENRE/${data.genrePath}`
      )
  ...
  _zoneGroupForDeviceId(deviceId) {
    ...
    if (!zoneGroup) {
      ...
      zoneGroup = this.zoneGroups.find(zg => {
        return (
          zg.members.findIndex(member => member.device.id === deviceId) > -1
        )
      })
    }
  ...
  async _getQueue(deviceId) {
    ...
    if (queue && queue.items) {
      queue.items.forEach((track, index) => {
        queue.items[index].queuePosition = index + 1
      })
      return queue.items
    }
    return null
  ...
  static _getGroupRenderingControlInfo(group) {
    ...
    const volume =
      typeof volumeSum === "object"
        ? volumeSum.volume
        : Math.floor(volumeSum / zones.length)
  ...
  async _getAVTransportInfo(deviceId) {
     ...
     * RecQualityMode
     */
    const transportSettings = await zone
      .avTransportService()
     ...
     */
    const transportActions = await zone
      .avTransportService()
    ...
    const tvPlaying =
      positionInfo.TrackURI.match(/^x-sonos-htastream:/) !== null
