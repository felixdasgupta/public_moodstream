...
const { NotASonosDevice, NoResultsFound } = require("./MusicLibraryErrors");
...
class MusicLibrary {
  ...
  constructor(Sonos) {
    this.sonos = Sonos;
    if (!this.sonos) {
      throw new Error(NotASonosDevice);
    }
  ...
  async browse(browseOptions) {
    ...
    try {
      ...
      const { category, term, separator } = MusicLibrary._getSearchOptions(
      ...
      // eslint-disable-next-line max-len
      const result = await this.sonos.searchMusicLibrary(
        category,
        searchTerm,
                ...
                try {
                  ...
                  if (item.title !== "All") {
                    if (result.items[index].albumArtURI === null) {
                      // eslint-disable-next-line max-len
                      result.items[index].albumArtURI = await this._getAlbumArt(
                        options.searchCategory,
                        item.title,
                        item.uri
                      );
  ...
  async getPlaylistURI(playlistName) {
    if (this._playlistCache) {
      const playlist = this._playlistCache.items.find(
  ...
  async getSonosPlaylists(searchOptions) {
    ...
    if (this._sonosPlaylistCache) {
      const items = this._sonosPlaylistCache.items.slice(
        options.start,
        options.total
      );
      return {
  ...
  async searchSonosPlaylists(options) {
    try {
      ...
      // eslint-disable-next-line max-len
      const results = playlists.items.filter(
        p =>
          p.title.toLowerCase().indexOf(options.searchTerm.toLowerCase()) !== -1
      );
      return {
        returned: String(results.length),
  ...
  _getCache(options) {
    const match = this._browseCache.find(cache => {
      if (
        options.searchCategory === cache.searchCategory &&
  ...
  async _getAlbumArt(searchCategory, searchTerm, uri) {
    ...
    try {
      ...
      // eslint-disable-next-line max-len
      let search = await this.sonos.searchMusicLibrary(
        category,
        decodeURI(term),
        {},
        separator
      ...
      if (searchCategory === "playlists") {
        ...
        const uniqueAlbums = [...new Set(songs.map(song => song.album))].slice(
          0,
          4
        );
